...
// We are adding a new method `getImageBufferViaSlice` that uses in-memory conversion of C array to go slice without using `C.GoBytes`.
// For minimum regression, this method is called only from `MultipageTIFFToPng`.
// In the subsequent release, we will knock off `getImageBuffer` and use `getImageBufferViaSlice` in other places as well.
func getImageBufferViaSlice(image *C.VipsImage, imageType ImageType) ([]byte, error) {
	length := C.size_t(0)
	interlace := C.int(0)
	compression := C.int(0)
	quality := C.int(100)
	...
	if length >= (1 << 50) {
		// Cannot represent in Golang as per https://github.com/golang/go/issues/13656#issuecomment-291957684
		return nil, fmt.Errorf("[getImageBufferViaSlice] Image too large to represent in Golang")
	}
